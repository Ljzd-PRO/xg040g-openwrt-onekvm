...
| 全核 RPS 与交换性能 LuCI | 64 个 RX 队列、状态与控制 RPC 已实机验证 |
...
当前正式版本为 [`v2026.07.14`](https://github.com/Ljzd-PRO/xg040g-openwrt-onekvm/releases/tag/v2026.07.14)。
详细状态见[硬件状态](docs/hardware-status.md)和[发行说明](docs/releases/v2026.07.14.md)。

> [!NOTE]
> 正式版包含“交换性能”页面、全核 RPS 默认值和可选 CPU 固定频率。2026-07-14
> 已完成无超频实机刷机验证：原厂 1200 MHz 策略不会加载 PLL 模块或访问时钟
> 寄存器；只有用户明确确认超频时才按需加载。当前包为
> `xg040g-performance 1.0.0-r5`，详细数据见
> [性能固件恢复与最终验证](docs/hardware-validation-20260714-performance-recovery.md)。
...
下载 `v2026.07.14` 正式版。不要从 Actions 的临时 artifact 或第三方转载地址
下载刷机镜像。
...
2026-07-14 的最终实机验证结果：

- `eth0` 和 `eth1` 各有 32 个 RX 队列，全部使用 RPS 掩码 `f`；
- “状态”“RPS 模式”和“恢复原厂 1200 MHz”操作均已验证；
- 默认配置为 `overclock=0`、`frequency=1200`，PLL 模块、sysfs 和自动加载项
  均不存在；
- 保留配置升级后四口 `br-lan`、LuCI、SSH 和 One-KVM 数据均保持正常。

普通用户保持“全核”和“1200 MHz”即可。“恢复原厂 1200 MHz”在默认状态下
不会加载 PLL 模块。1300/1400 MHz 会要求输入 `OVERCLOCK`，属于未完成长期
压力验证的实验功能，不建议用于远程无人值守设备。

CPU 发布默认始终为原厂 1200 MHz，此时实验性 PLL 驱动保持未加载。
1300/1400 MHz 只作为带确认警告和温控回退的实验选项；远程无人值守设备建议保持
1200 MHz。本固件不启用已知会导致 DSA 到 2.5G 方向丢包的 NPU/PPE TC
flower 加速。
...
- sysupgrade 或重启后，上游 DHCP 可能给同一设备分配不同 IPv4 地址。不要只
  反复尝试旧地址，应按 `xg040g-xxxxxx.local` 或租约中的稳定 MAC 查找。

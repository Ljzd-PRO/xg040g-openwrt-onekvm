...
host-only IP-KVM 和小型交换机使用：MS2109 采集目标电脑的 HDMI 画面，
CH340/CH341 + CH9329 模拟键盘鼠标，LAN4 为独立 PXE 端口。
...
当前 Release 在 USB3 `KVMSTORE` 完成硬件闭环前保持 prerelease。详细状态见
[硬件状态](docs/hardware-status.md)。
...
| 端口 | 默认用途 |
| --- | --- |
| LAN2 | 管理交换机端口；也是 OpenWrt failsafe 和 tcboot 恢复口 |
| LAN3 | 管理交换机端口 |
| 2.5G | 管理交换机端口，可连接上游网络 |
| LAN4 | 专用 PXE 端口，固定服务地址 `10.40.0.1`，不是管理口 |
...
> [!WARNING]
> LAN4 会主动提供独立 PXE DHCP，不能连接家庭、办公或数据中心的上游网络。
> LAN2、LAN3 和 2.5G 管理交换桥默认关闭 STP，也不要用多根网线把它们接回
> 同一个交换机形成物理环路。

## 下载固件

从 [GitHub Releases](https://github.com/Ljzd-PRO/xg040g-openwrt-onekvm/releases)
下载固件。若当前只有 prerelease，需要在 Releases 页面展开预发布版本。
...
1. 将电脑或上游路由器连接 LAN2、LAN3 或 2.5G 口，不要连接 LAN4。
...
LAN2、LAN3 和 2.5G 口组成 `br-lan` 透明交换桥。固件不是传统的 WAN/LAN
路由器，不存在默认 WAN、LAN 到 WAN NAT 或 DHCP Server。LAN4 独立为
`br-pxe`，只给 PXE 客户端分配 `10.40.0.100-10.40.0.200`。

![LuCI 网络接口：br-lan DHCP 管理与独立 br-pxe](docs/images/luci-network-interfaces.png)
...
PXE 只使用 LAN4。目标电脑应启用网卡启动，x86-64 UEFI 客户端会下载
`ipxe.efi`，传统 BIOS 客户端会下载 `undionly.kpxe`，随后通过 HTTP 打开
`boot.ipxe` 菜单。
...
1. 确认 `http://10.40.0.1:8081/boot.ipxe` 可以从 LAN4 网络读取。
2. 将目标电脑网线接到 XG 的 LAN4。
3. 在目标电脑固件设置中关闭 Secure Boot，并把 UEFI Network/PXE 放到启动
   顺序前面；老机器使用 Legacy PXE。
4. 客户端应取得 `10.40.0.x`，随后显示 XG-040G-MD iPXE 菜单。
5. 本地模式默认不需要上游网络，断网也可以使用已有文件。

## 无硬盘 Cloud PXE

Cloud PXE 使用 `rclone serve http --read-only`，由 XG 从远程存储读取文件并
在 LAN4 的 `10.40.0.1:8083` 提供给 iPXE。支持 WebDAV、S3、SFTP、OneDrive、
Google Drive 以及当前 rclone 版本支持的其他 remote 类型。

### 接线要求

Cloud PXE 同时需要两条网络链路：

1. LAN2、LAN3 或 2.5G 连接上游网络，让 XG 能访问云存储。
2. LAN4 连接需要 PXE 启动的目标电脑。

只有一根网线时可以配置和测试 remote，也可以单独验证 LAN4 PXE，但不能同时
完成“XG 联网取云端文件”和“目标电脑从 LAN4 启动”的完整链路。
...
1. 在“活动远程路径”填写具名路径，例如 `kvmcloud:/Downloads`。
2. 将 rclone INI 配置粘贴到代码输入框。
3. 点击“测试未保存配置”，确认 remote 存在且能列举目录。
4. 勾选“启用云端 PXE”，点击“保存并应用”。
5. 状态应变成 Running，数据面地址为 `http://10.40.0.1:8083`。
6. 将目标电脑接到 LAN4，并按前述 UEFI/BIOS PXE 步骤启动。
...
开关只决定 LAN4 客户端能否经 NAT 访问上游。如果 FirPE 网络模式或启动后的
应急系统需要联网，可以在“服务 -> One-KVM -> 常规”临时开启，使用完再关闭。
...
和 KVMSTORE 内容会保留；新网络 schema 第一次运行仍会覆盖旧网络拓扑。
...
LAN4 的 `10.40.0.1` 永远不是 LuCI、SSH 或恢复入口。完整说明见
[网络与恢复](docs/network-recovery.md)。

## 故障排查

### 找不到管理地址

- 确认网线在 LAN2、LAN3 或 2.5G，而不是 LAN4。
...
确认目标电脑连接 LAN4、Secure Boot 已关闭、网卡 PXE 已启用。LAN2/LAN3/
2.5G 不提供 PXE DHCP。UEFI 客户端应请求 `ipxe.efi`，Legacy BIOS 请求
`undionly.kpxe`。

...
> 不包含在已经发布的 `v2026.07.13-rc1` 中。2026-07-14 已完成无超频实机
> 刷机验证：原厂 1200 MHz 策略不会加载 PLL 模块或访问时钟寄存器；只有用户
> 明确确认超频时才按需加载。当前已验证包为 `xg040g-performance 1.0.0-r5`，
> 详细数据见[性能固件恢复与最终验证](docs/hardware-validation-20260714-performance-recovery.md)。

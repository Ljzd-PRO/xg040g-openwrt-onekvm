...
for pxe_port in none lan2 lan3 lan4 eth1; do
	...
	mapfile -t memberships < <(awk '$1 == "list" && $2 == "ports" { gsub(/\047/, "", $3); print $3 }' "$dir/network" | sort)
	[[ "${memberships[*]}" == 'eth1 lan2 lan3 lan4' ]] || fail "$pxe_port does not assign every physical port exactly once"

	mapfile -t lan_ports < <(awk '
	...
	if [[ "$pxe_port" == none ]]; then
		...
		expected="$(printf '%s\n' eth1 lan2 lan3 lan4 | grep -Fxv "$pxe_port" | sort | paste -sd' ' -)"

...
独立的“PXE 设置”页面可持久选择任意一个物理网口，并控制该 PXE 客户端的
上游访问。默认不选择端口，四口均为交换机口；上游访问也默认关闭。选择端口后
只允许本地 DHCP/TFTP/HTTP；开启上游访问才创建单向 `pxe -> lan` 转发并在
管理出口做 NAT，不会开放上游到 PXE 子网或 PXE 到设备管理端口的访问。

...
- [x] 默认 1200 MHz、超频锁关闭、PLL 模块不自动加载及“恢复原厂频率”
  不加载模块已完成实机验证。
- [x] `eth0`/`eth1` 共 64 个 RX 队列的全核 RPS 掩码、LuCI 状态 RPC 和
  参数化控制 RPC 已在修正版固件上验证。
- [ ] 动态 PXE 切换后的 RPS 重载与持续多流量回归。
...
核心默认路径通过实机验证后可以发布正式版。未完成的可选硬件、实验功能和性能
压力测试必须在 README 与发行说明中逐项披露，不得描述为已验证。

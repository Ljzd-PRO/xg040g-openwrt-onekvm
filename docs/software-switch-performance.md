...
专用内核模块只支持 `nokia,xg-040g-md-tcboot`，但默认不自动加载。在
1200 MHz 原厂策略下，系统不映射 PLL 寄存器，LuCI 会显示“PLL 驱动
未加载”。只有用户输入 `OVERCLOCK` 后，固件才按需加载模块，先确认
读回值是 1200/1300/1400 MHz 之一，再允许修改频率。该路径不修改 CPU 电压。
模块卸载时只会在读回值明确为 1300/1400 MHz 时恢复 1200 MHz；异常读回不会
触发任何 PLL 写入。
...
- 温度或 PLL 回读异常：拒绝继续写寄存器；
- 发现上次异常结束标记：下次启动保持原厂策略，不加载 PLL 模块。
...
高于 1200 MHz 的命令仍会检查温度、频率白名单和 PLL 回读，并且是唯一会
按需加载 PLL 驱动的路径。
...
固件默认 1400 MHz，而本固件始终默认 1200 MHz，原厂策略下甚至不自动加载
PLL 模块，仅保留手动实验入口。
